...
cc.Class({
    ...
    onLoad() {
        if (comment.query.length !== 0) {
            let roomID = comment.query[0];
            let maxPlayer = comment.query[1];
            let mode = comment.query[2];
            this.friendJoinRoom(roomID, maxPlayer, mode);
        }
    },

    friendJoinRoom: function (roomID, maxPlayer, mode) {
        this.maxPlayer = maxPlayer;
        this.mode = mode;
        this.loadInit();
        engine.joinRoom(roomID, "Hello world");
    },
    ...
    onShareBtn: function () { //分享按钮	
        let roomID = roomMsg.roomID;
        let maxPlayer = this.maxPlayer;
        let mode = this.mode;
        cc.loader.loadRes("", function () {
            wx.shareAppMessage({
                title: "不怕，就来圈地大作战PK！",
                query: "roomID=" + roomID + "&maxplayer=" + maxPlayer + "&mode=" + mode,
                imageUrl: canvas.toTempFilePathSync({
                    destWidth: 500,
                    destHeight: 400
                }),
                success() {
                    self.showTipsUI("转发成功!!!");
                },
                fail() {
                    self.showTipsUI("转发失败!!!");
                }
            })
        });
    },
    ...
    //创建房间
    createRoom: function (matchinfo) {
        let userIdStr = "engine.mUserID.toString()";
        engine.createRoom(matchinfo, userIdStr);
    },
    ...
    createRoomResponse: function (CreateRoomRsp) {
        if (CreateRoomRsp.status === 200) {
            //保存房间相关的信息
            roomMsg = CreateRoomRsp;
            if (this.isOwner(engine.mUserID)) {
                userList[0] = useInfo;
                this.listToBox();
            }
    ...
    leaveRoomNotifyRoom: function (leaveRoomInfo) {
        roomMsg.owner = leaveRoomInfo.owner;
        this.drawRoom();
    ...
    //初始化实时对战模式的房间信息
    initMacthinfo: function (maxPlayer, mode) {
        let matchinfo = new matchvs.MsCreateRoomInfo()
        matchinfo.maxPlayer = maxPlayer;
        matchinfo.mode = mode;
        matchinfo.canWatch = 2;
        matchinfo.visibility = 1;
        matchinfo.roomProperty = "房间模式";
        return matchinfo;
    },
    ...
    drawRoom: function () {
        ...
        roomOwner.getComponent(cc.Label).string = "房主信息：" + roomMsg.owner;
    ...
    listToBox: function () {
        for (let i = 0; i < this.maxPlayer; i++) {
            ...
            } else if (userList[i].username !== undefined) {
                id.getComponent(cc.Label).string = userList[i].username;
            } else {
                id.getComponent(cc.Label).string = "XXX";
            }
